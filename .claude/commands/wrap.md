...
7. Check the merge gates. All of these must hold, or stop and ask instead of merging — a failed or ambiguous gate is not a reason to proceed quietly:
   ...
   - **Manual-check list cleared or explicitly waived**: if `/test` produced a NEEDS MANUAL CHECK list, either the user has confirmed those items directly, or the user has explicitly said to proceed without them (on the record in this conversation, not inferred). Otherwise stop and ask — don't merge past an unaddressed manual-check list.
